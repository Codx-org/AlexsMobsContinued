...
public class ItemEcholocator extends Item {
    ...
    private List<BlockPos> getNearbyPortals(BlockPos blockpos, ServerLevel world, int range) {
        if(type == EchoType.ENDER){
            ...
            Stream<BlockPos> stream = pointofinterestmanager.findAll(poiTypeHolder -> poiTypeHolder.is(AMPointOfInterestRegistry.END_PORTAL_FRAME_KEY), Predicates.alwaysTrue(), blockpos, range, PoiManager.Occupancy.ANY);
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player livingEntityIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide() && worldIn instanceof ServerLevel) {
            ...
            if(type == EchoType.ENDER){
                ...
                CompoundTag nbt = AMCompat.getOrCreateTag(stack);
                if(AMCompat.contains(nbt, "CavePos") && AMCompat.getBoolean(nbt, "ValidCavePos")){
                    pos = BlockPos.of(AMCompat.getLong(nbt, "CavePos"));
                    if(isCaveAir(worldIn, pos) || 1000000 < pos.distSqr(playerPos)){
                        nbt.putBoolean("ValidCavePos", false);
                        AMCompat.setTag(stack, nbt);
                    }
                }else{
                    for (BlockPos portalPos : portals) {
                        if (pos == null || pos.distSqr(playerPos) < portalPos.distSqr(playerPos)) {
        ...
        com.github.alexthe666.alexsmobs.misc.AMCompat.addCooldown(livingEntityIn.getCooldowns(), this, 5);

        return AMCompat.sidedSuccess(stack, worldIn.isClientSide());
    }
