...
public class MovementControllerCustomCollisions extends MoveControl {
    ...
    private boolean isWalkable(float p_234024_1_, float p_234024_2_) {
        ...
        if (pathnavigator != null) {
            ...
            //? if >=1.20.5 {
            /*// 1.20.5 replaced the (level, x, y, z) lookup with a PathfindingContext-based one;
            // the (mob, pos) overload builds that context itself.
            if (nodeprocessor != null && nodeprocessor.getPathType(this.mob, net.minecraft.core.BlockPos.containing(this.mob.getX() + (double)p_234024_1_, this.mob.getY(), this.mob.getZ() + (double)p_234024_2_)) != BlockPathTypes.WALKABLE) {
            *///?} else {
