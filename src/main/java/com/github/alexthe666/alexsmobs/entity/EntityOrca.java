...
public class EntityOrca extends TamableAnimal implements IAnimatedEntity {
    ...
    *///?} else {
    ...
    public void tick() {
        ...
        if (attackTarget != null && distanceTo(attackTarget) < attackTarget.getBbWidth() + this.getBbWidth() + 2) {
            if (this.getAnimation() == ANIMATION_BITE && this.getAnimationTick() == 4) {
                ...
                boolean flag = AMCompat.hurt(attackTarget, this.damageSources().mobAttack(this), damage);
                if (flag) {
                    AMCompat.enchantDamageEffects(this, attackTarget);
            ...
            if (this.getAnimation() == ANIMATION_TAILSWING && this.getAnimationTick() == 6) {
                ...
                final float yRotRad = this.getYRot() * Mth.DEG_TO_RAD;
                AMCompat.knockback(attackTarget, 1F, Mth.sin(yRotRad), -Mth.cos(yRotRad));
                float knockbackResist = (float) Mth.clamp((1.0D - this.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE)), 0, 1);
                /* Use the local, not a second getTarget(): the hurt above can clear this mob's target
                 * (the victim dies, or a knockback listener retargets it) between the null check at the
                 * top of this block and here, and upstream's re-read then NPEs mid-tick. */
                attackTarget.setDeltaMovement(attackTarget.getDeltaMovement().add(0, knockbackResist * 0.4F, 0));

            }
        }
        if (attackTarget != null && attackTarget instanceof Player && attackTarget.hasEffect(AMCompat.effect(AMEffectRegistry.ORCAS_MIGHT.get()))) {
            attackTarget.removeEffect(AMCompat.effect(AMEffectRegistry.ORCAS_MIGHT.get()));
        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob p_241840_2_) {
        return AMCompat.create(AMEntityRegistry.ORCA.get(), serverWorld);
    }
    ...
    *///?} else {
    ...
    public void onJumpHit(LivingEntity entityIn) {
        boolean flag = AMCompat.hurt(entityIn, this.damageSources().mobAttack(this), (float) ((int) this.getAttributeValue(Attributes.ATTACK_DAMAGE)));
        if (flag) {
            AMCompat.enchantDamageEffects(this, entityIn);
            this.playSound(SoundEvents.DOLPHIN_ATTACK, 1.0F, 1.0F);
        }
    }
    ...
    static class SwimWithPlayerGoal extends Goal {
        ...
        public boolean canUse() {
            this.targetPlayer = AMCompat.getNearestPlayer(this.dolphin.level(), EntityOrca.PLAYER_PREDICATE, this.dolphin);
        ...
        public void tick() {
            ...
            if (this.targetPlayer.isSwimming() && this.targetPlayer.level().getRandom().nextInt(6) == 0) {
                this.targetPlayer.addEffect(new MobEffectInstance(AMCompat.effect(AMEffectRegistry.ORCAS_MIGHT.get()), 1000));
            }
        }
    }
