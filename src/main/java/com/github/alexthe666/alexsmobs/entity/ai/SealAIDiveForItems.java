...
public class SealAIDiveForItems extends Goal {
    ...
    public SealAIDiveForItems(EntitySeal seal) {
        this.seal = seal;
        // UPSTREAM FIX (#83): this goal declared no flags, so it reserved nothing. It re-paths every
        // tick, but RandomSwimmingGoal (priority 7) and LookAtPlayerGoal (8) were free to run
        // alongside it and kept yanking the seal off the dig site — and digTime only advances while
        // the seal is actually within 2 blocks of it, so the 100-tick dig could take minutes or
        // never finish. MOVE puts it above both of them; the goals that legitimately outrank it
        // (bask 0, breathe 1, breed 2, find/leave water 3, panic 4, melee 5) all still do, and
        // AnimalAIFindWater at 3 is exactly how the seal now reaches the sea in the first place.
        this.setFlags(EnumSet.of(Flag.MOVE, Flag.LOOK));
    }

    private static List<ItemStack> getItemStacks(EntitySeal seal) {
        LootTable loottable = com.github.alexthe666.alexsmobs.misc.AMCompat.lootTable(seal.level().getServer(), SEAL_REWARD);
    ...
    public boolean canUse() {
        ...
        // genDigPos() is a 15-attempt column scan, and the goal selector polls every tick for as
        // long as the seal owes a treasure — which, with the seal now walking to the sea to look for
        // one (#83), is a good deal longer than it used to be. Only look every half second.
        if (searchCooldown > 0) {
            searchCooldown--;
            return false;
        }
    ...
    public void tick() {
        ...
        if (returnToPlayer) {
            ...
            if (seal.distanceTo(thrower) < 2D) {
                ...
                ItemEntity item = AMCompat.spawnAtLocation(seal, stack);
                if (item != null) {
    ...
    public void stop() {
        ...
        if(!seal.getMainHandItem().isEmpty()){
            AMCompat.spawnAtLocation(seal, seal.getMainHandItem().copy());
            seal.setItemInHand(InteractionHand.MAIN_HAND, ItemStack.EMPTY);
        }
    }
