...
public class MantisShrimpAIBreakBlocks extends Goal {
    ...
    private void breakBlock() {
        if (shouldMoveTo(mantisShrimp.level(), destinationBlock)) {
            ...
            // The fabric arm is FIRST on purpose: >=1.21.2 is also true on the 26.2 Fabric node, and
            // whichever arm matches first wins. Fabric has no canEntityDestroy hook (no protection
            // mods to consult), so the check is simply dropped there.
            //? if fabric {
            /*if(!mantisShrimp.level().isEmptyBlock(destinationBlock) && state.getDestroySpeed(mantisShrimp.level(), destinationBlock) >= 0){
            *///?} elif >=1.21.2 {
            /*if(!mantisShrimp.level().isEmptyBlock(destinationBlock) && net.minecraftforge.common.ForgeHooks.canEntityDestroy((net.minecraft.server.level.ServerLevel) mantisShrimp.level(), destinationBlock, mantisShrimp) && state.getDestroySpeed(mantisShrimp.level(), destinationBlock) >= 0){
            *///?} else {
