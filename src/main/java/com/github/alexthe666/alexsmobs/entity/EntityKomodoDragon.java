...
public class EntityKomodoDragon extends TamableAnimal implements ITargetsDroppedItems, IFollower {
    ...
    private static final java.util.function.Supplier<Ingredient> TEMPTATION_ITEMS = AMCompat.lazyIngredient(() -> AMCompat.ingredientOf(AMTagRegistry.KOMODO_DRAGON_TAMEABLES));
    ...
    *///?} else {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide() && this.isAlive() && !this.isBaby() && --this.timeUntilSpit <= 0) {
            AMCompat.spawnAtLocation(this, AMItemRegistry.KOMODO_SPIT.get());
