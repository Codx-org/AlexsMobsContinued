...
public class EntityAlligatorSnappingTurtle extends Animal implements ISemiAquatic, Shearable, net.minecraftforge.common.IForgeShearable {
    ...
    *///?} else {
    ...
    // AMCompat#shearWithShears. This mob overrides nothing else here, so the whole method is
    // Fabric's; on the other loaders the shearable interface above carries it.
    //? if fabric {
    /*@Override
    public net.minecraft.world.InteractionResult mobInteract(Player player, net.minecraft.world.InteractionHand hand) {
        if (AMCompat.shearWithShears(this, player, hand, player.getItemInHand(hand))) {
    ...
    public void shear(SoundSource category) {
        ...
        if (!this.level().isClientSide()) {
            if (random.nextFloat() < this.getMoss() * 0.05F) {
                AMCompat.spawnAtLocation(this, AMItemRegistry.SPIKED_SCUTE.get());
            } else {
