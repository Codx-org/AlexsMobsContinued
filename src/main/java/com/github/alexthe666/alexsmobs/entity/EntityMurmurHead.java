...
public class EntityMurmurHead extends Monster implements FlyingAnimal {
    ...
    // 1.21.2 deleted Entity#walkDist and #walkDistO (javap-checked: present at 1.21.1, gone at
    // 1.21.2), which ModelMurmurHead read to swing the hair. The port swapped that for
    // walkAnimation.position(), and those are not the same quantity: walkDist accumulates
    // horizontalDistance * 0.6 per move, while WalkAnimationState accumulates a smoothed speed that
    // saturates at 1.0 per tick — roughly seven times faster — so the hair whipped instead of
    // swaying. Reported as "the hair on the murmurs moves in a somewhat strange way" (#68).
    //
    // Vanilla hit exactly this in its own cape animation (the maths upstream copied here, sin(d*6)
    // * 32) and answered it the same way: ClientAvatarState now keeps the accumulator itself and
    // AvatarRenderer reads getInterpolatedWalkDistance(partialTick). These two fields are that,
    // scoped to this entity, so every node runs one uncontested code path.
    ...
    *///?} else {
    ...
    // Entity#baseTick did `walkDistO = walkDist` before anything moved; tick() calls baseTick first,
    // so the top of this method is the same instant. See the fields' note (#68).
    @Override
    public void move(net.minecraft.world.entity.MoverType type, Vec3 movement) {
        ...
        // Upstream added `vec3.horizontalDistance() * 0.6` for the movement that actually resolved
        // after collision, which is precisely the position delta across the super call. Reading it
        // this way needs no access to the local vanilla kept it in.
        this.amWalkDist += (float) Math.sqrt(dx * dx + dz * dz) * 0.6F;
    }
    ...
    public boolean hurt(DamageSource source, float damage) {
        ...
        if(body != null && AMCompat.hurt(body, source, 0.5F * damage)){
            return true;
        }
