...
public class EntityManedWolf extends Animal implements ITargetsDroppedItems, IDancingMob {
    ...
    private static final java.util.function.Supplier<Ingredient> allFoods = AMCompat.lazyIngredient(() -> AMCompat.ingredientOfTags(AMTagRegistry.MANED_WOLF_BREEDABLES, AMTagRegistry.MANED_WOLF_STENCH_FOODS));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new TemptGoal(this, 1.1D, allFoods.get(), false));
    ...
    *///?} else {
    ...
    private void findAnthill(){
        if(nearestAnthill == null || !(level().getBlockEntity(nearestAnthill) instanceof TileEntityLeafcutterAnthill)){
            ...
            Stream<BlockPos> stream = pointofinterestmanager.findAll((poiTypeHolder -> poiTypeHolder.is(AMPointOfInterestRegistry.LEAFCUTTER_ANT_HILL_KEY)), Predicates.alwaysTrue(), this.blockPosition(), 10, PoiManager.Occupancy.ANY);
