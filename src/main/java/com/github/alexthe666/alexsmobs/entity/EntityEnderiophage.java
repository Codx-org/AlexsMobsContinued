...
public class EntityEnderiophage extends Animal implements Enemy, FlyingAnimal {
    ...
    *///?}
    ...
    private static final Predicate<LivingEntity> ENDERGRADE_OR_INFECTED = (entity) -> {
        return entity instanceof EntityEndergrade || entity.hasEffect(AMCompat.effect(AMEffectRegistry.ENDER_FLU.get()));
    };
            ...
            if (this.isPassenger()) {
                ...
                if (mount instanceof LivingEntity) {
                    ...
                    if (!this.level().isClientSide() && attachTime > 15) {
                        ...
                        if ((target.getHealth() < 1.5D || AMCompat.hurt(mount, this.damageSources().mobAttack(this), dmg)) && mount instanceof LivingEntity) {
            ...
            if (fleeAfterStealTime > 0) {
                if (angryEnderman != null) {
                    ...
                    if (fleeAfterStealTime < 5) {
                        ...
                        try {
                            AMCompat.stopRunningGoals(angryEnderman.goalSelector);
                            AMCompat.stopRunningGoals(angryEnderman.targetSelector);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        angryEnderman = null;
