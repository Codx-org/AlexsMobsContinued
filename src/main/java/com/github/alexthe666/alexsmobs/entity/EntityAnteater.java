...
public class EntityAnteater extends Animal implements NeutralMob, IAnimatedEntity, ITargetsDroppedItems {
    ...
    *///?} else {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.2D, AMCompat.ingredientOf(AMTagRegistry.ANTEATER_FOODSTUFFS), false));
    ...
    // The baby rides on its mother's back (AnimalAIRideParent) and upstream never declared a seat,
    // so it sat wherever vanilla's default put it: 0.75 of the vehicle's height. 1.20.2 deleted
    // Entity#getPassengersRidingOffset and the EntityAttachment.PASSENGER fallback that replaced it
    // is AT_HEIGHT, i.e. the FULL 1.1 — so on every node above 1.20.1 the baby floats a quarter of
    // a block above its mother's back. Restate upstream's old default explicitly.
    //? if >=1.20.2 {
    /*@Override
    public net.minecraft.world.phys.Vec3 getPassengerRidingPosition(Entity passenger) {
        return new net.minecraft.world.phys.Vec3(this.getX(), this.getY() + (double) this.getBbHeight() * 0.75D, this.getZ());
    }
    *///?}
    ...
    *///?} else {
                ...
                if (distanceTo(attackTarget) < attackTarget.getBbWidth() + this.getBbWidth() + 2) {
                    if (this.getAnimationTick() == 7) {
                        if (this.getAnimation() == ANIMATION_SLASH_L) {
                            ...
                            AMCompat.knockback(attackTarget, 0.5F, Mth.sin(rot * Mth.DEG_TO_RAD), -Mth.cos(rot * Mth.DEG_TO_RAD));
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob parent) {
        return AMCompat.create(AMEntityRegistry.ANTEATER.get(), level());
    }
