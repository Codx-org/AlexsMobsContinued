...
public class RockyChestplateUtil {
    ...
    public static int getRollingTicksLeft(LivingEntity entity) {
        ...
        if (lassoedTag.contains(ROCKY_ROLL_TICKS)) {
            return AMCompat.getInt(lassoedTag, ROCKY_ROLL_TICKS);
        }
        return 0;
    }
    ...
    public static int getRollingTimestamp(LivingEntity entity) {
        ...
        if (lassoedTag.contains(ROCKY_ROLL_TIMESTAMP)) {
            return AMCompat.getInt(lassoedTag, ROCKY_ROLL_TIMESTAMP);
        }
        return 0;
    }
    ...
    public static void tickRockyRolling(LivingEntity roller) {
        ...
        if(rollCounter == 0){
            ...
            // getForcedPose/setForcedPose are Forge IForgeEntity extensions with no vanilla
            // equivalent. On Fabric the rolling player therefore keeps their normal standing pose
            // instead of being forced into the swimming (horizontal) one — cosmetic only.
            //? if !fabric {
