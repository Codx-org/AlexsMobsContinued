...
public class EntitySpectre extends Animal implements FlyingAnimal {
    ...
    *///?} else {
    ...
    // Upstream inlined Mob#tickLeash's tail (restore-from-save + liveness check) so it could
    // slip a long-range pull in between. 1.20.5 made both the saved-leash field and
    // restoreLeashFromSave private, so delegate to super whenever there is no live holder —
    // with holder == null that IS exactly the restore-and-check tail, in every version.
    // 1.21 moved leashing into Leashable: there is no tickLeash to override, and the per-mob hook
    // fires only once a live holder exists (so the null case below can't happen there). Returning
    // true hands control back to the default pull, which is what delegating to super meant.
    // 1.21.6 split that hook into staged ones and gave Leashable a snap distance (12) and an
    // elastic distance (6) a mob can override. Upstream's spectre neither breaks its lead nor gets
    // pulled below 30 blocks, so it opts out of both vanilla bands — a snap distance far past any
    // reachable range, an elastic distance of 30 — and does its own long-range yank from
    // onElasticLeashPull, the only hook that then fires. A holder that is a passenger or a fence
    // knot still gets vanilla's handling, as returning true used to mean.
    //? if >=1.21.6 {
    /*@Override
    ...
    public void onElasticLeashPull() {
        ...
        if (holder == null || holder.isPassenger() || holder instanceof LeashFenceKnotEntity) {
            super.onElasticLeashPull();
            return;
        }
        amTickLeash(this.distanceTo(holder));
    }
    *///?} elif >=1.21 {
    /*@Override
    ...
    *///?} else {
    protected void tickLeash() {
        ...
        amTickLeash(this.distanceTo(this.getLeashHolder()));
    }
    //?}
    ...
    static class TemptHeartGoal extends Goal {
        ...
        public boolean canUse() {
            if (this.delayTemptCounter > 0) {
                ...
                this.closestPlayer = AMCompat.getNearestPlayer(this.creature.level(), ENTITY_PREDICATE, this.creature);
