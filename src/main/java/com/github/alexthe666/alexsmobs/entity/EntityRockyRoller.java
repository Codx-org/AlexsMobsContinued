    ...
    *///?} else {
    ...
    private void earthquake() {
        ...
        if (!this.level().canSeeSky(this.blockPosition()) && AMCompat.gameRule(this.level(), AMCompat.Rule.MOB_GRIEFING)) {
            ...
            while ((!level().getBlockState(ceil).isSolid() || level().getBlockState(ceil).getBlock() == Blocks.POINTED_DRIPSTONE) && ceil.getY() < AMCompat.maxBuildHeight(level())) {
                ceil = ceil.above();
            }
            ...
            for (BlockPos blockpos1 : BlockPos.betweenClosed(ceil.offset(-i, -j, -k), ceil.offset(i, j, k))) {
                if (blockpos1.distSqr(ceil) <= fTimesF && level().getBlockState(blockpos1).getBlock() instanceof Fallable) {
                    if (isHangingDripstone(blockpos1)) {
                        while (isHangingDripstone(blockpos1.above()) && blockpos1.getY() < AMCompat.maxBuildHeight(level())) {
    ...
    // See AMCompat.isFullyConstructed — Moonrise asks this from inside Entity.<init>.
    public boolean canBeCollidedWith() {
        return AMCompat.isFullyConstructed(this) && this.isAlive();
    }

    /**
     * Maximum momentum, in blocks/tick, this boulder will hand to something it rolls into.
     * Comfortably above the roller's own top speed, so the flung-across-the-room feel is intact.
     */
    private static final double MAX_PUSH_IMPULSE = 2.0D;

    public void push(Entity entity) {
        // Vanilla Entity#push caps the impulse it transfers (0.05 per axis). This override
        // deliberately does not, so a charging boulder can fling what it hits — fine one-way, but
        // between TWO rollers it is symmetric AND unbounded: each adds the other's full velocity
        // to its own every tick, so the pair's speed grows by the golden ratio per tick and is
        // astronomical within a couple of seconds. The swept AABB that
        // ICustomCollisions#getAllowedMovementForEntity then has to walk spans millions of blocks
        // and wedges the server thread -> "java.lang.Error: Watchdog", top frame
        // BlockBehaviour#getCollisionShape. Rollers spawn in groups, which is why it only ever
        // happened "sometimes". So: fall back to vanilla's capped push between rollers, and clamp
        // what is transferred to anything else.
        ...
        if (length > MAX_PUSH_IMPULSE) {
            impulse = impulse.scale(MAX_PUSH_IMPULSE / length);
        }
    ...
    static class RockyRollerNodeEvaluator extends WalkNodeEvaluator {
        ...
        public BlockPathTypes getPathType(net.minecraft.world.level.pathfinder.PathfindingContext context, int x, int y, int z) {
            ...
            return context.getBlockState(pos).getBlock() instanceof PointedDripstoneBlock ? BlockPathTypes.OPEN : super.getPathType(context, x, y, z);
        }
        *///?} else {
