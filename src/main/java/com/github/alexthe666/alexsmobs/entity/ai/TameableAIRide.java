...
public class TameableAIRide extends Goal {
    ...
    @Override
    public void tick() {
        AMCompat.setMaxUpStep(tameableEntity, 1F);
        tameableEntity.getNavigation().stop();
        ...
        if (strafe) {
            // This is a Goal, i.e. server-side only, and 1.21.2 stopped populating Player#xxa/zza
            // there — read the rider's input through AMCompat or every ridden tameable stops
            // responding to its rider's movement keys. Same for the two predicates below.
            tameableEntity.xxa = AMCompat.riderStrafe(player) * 0.15F;
        }
    ...
    public boolean shouldMoveForward() {
        return AMCompat.riderForward(player) != 0;
    }
