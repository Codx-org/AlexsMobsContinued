...
public class EntityStraddler extends Monster implements IAnimatedEntity {
    ...
    *///?} else {
    ...
    public net.minecraft.world.phys.shapes.VoxelShape getLiquidCollisionShape() {
        return net.minecraft.world.level.block.Block.column(16.0D, 0.0D, 8.0D);
    }
    *///?}
    ...
    private void floatStrider() {
        if (this.isInLava()) {
            ...
            double d1 = com.github.alexthe666.alexsmobs.misc.AMPlatform.fluidHeightLava(this);
            if(d1 <= 0.5F && d1 > 0){
            ...
            //? if >=26 {
            /*}else if (lvt_1_1_.isAbove(this.getLiquidCollisionShape(), this.blockPosition().below(), true) && !this.level().getFluidState(this.blockPosition().above()).is(FluidTags.LAVA)) {
            *///?} elif >=1.21.5 {
            /*}else if (lvt_1_1_.isAbove(LiquidBlock.SHAPE_STABLE, this.blockPosition().below(), true) && !this.level().getFluidState(this.blockPosition().above()).is(FluidTags.LAVA)) {
            *///?} else {
    ...
    public void tick() {
        ...
        if (this.getAnimation() == ANIMATION_LAUNCH && this.isAlive() && this.getAnimationTick() == 20 && this.getTarget() != null) {
            EntityStradpole pole = AMCompat.create(AMEntityRegistry.STRADPOLE.get(), level());
    ...
    static class LavaPathNavigator extends GroundPathNavigation {
        ...
        // MC 26.2 deleted GroundPathNavigation#hasValidPathType outright — path-type validity is
        // decided by the node evaluator now. Vanilla's Strider, which this navigator is copied
        // from, dropped its identical override in the same version, so following it is faithful.
        //? if <26.2 {
