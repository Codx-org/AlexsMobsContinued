...
public class EntityGiantSquidPart extends PartEntity<EntityGiantSquid> implements IHurtableMultipart {
    ...
    // does not implement — nothing leashes a squid arm, so the body stays as harmless dead code.
    //? if <1.21.6
    @Override
    ...
    public boolean hurt(DamageSource source, float amount) {
        if(this.level().isClientSide() && this.getParent() != null && !AMCompat.isInvulnerableTo(this.getParent(), source) && !collisionOnly){
            AlexsMobs.sendMSGToServer(new MessageHurtMultipart(this.getId(), this.getParent().getId(), amount, source.getMsgId()));
        }
        return !collisionOnly && !AMCompat.isInvulnerableTo(this, source) && this.getParent().attackEntityPartFrom(this, source, amount);
    }
