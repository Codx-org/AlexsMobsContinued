...
public class EntityFly extends Animal implements FlyingAnimal {
    ...
    *///?} else {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new TemptGoal(this, 1.25D, AMCompat.ingredientOfTags(AMTagRegistry.FLY_BREEDABLES, AMTagRegistry.FLY_FOODSTUFFS), false));
    ...
    *///?} else {
    ...
    public void tick(){
        ...
        if(isInNether()){
            ...
            if(conversionTime > 300){
                EntityCrimsonMosquito mosquito = AMCompat.create(AMEntityRegistry.CRIMSON_MOSQUITO.get(), level());
                mosquito.copyPosition(this);
                if(!this.level().isClientSide()){
                    //? if >=1.20.5 {
                    /*mosquito.finalizeSpawn((ServerLevelAccessor)level(), AMCompat.difficultyAt(level(), this.blockPosition()), MobSpawnType.CONVERSION, null);
                    *///?} else {
                    mosquito.finalizeSpawn((ServerLevelAccessor)level(), AMCompat.difficultyAt(level(), this.blockPosition()), MobSpawnType.CONVERSION, null, null);
                    //?}
                }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob parent) {
        EntityFly fly = AMCompat.create(AMEntityRegistry.FLY.get(), level());
