...
public class EntityUnderminer extends PathfinderMob {
    ...
    *///?} else {
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        if(lastGivenStack != null){
            AMCompat.put(compound, "MineStack", com.github.alexthe666.alexsmobs.misc.AMCompat.saveItem(this.level().registryAccess(), lastGivenStack));
        }
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if(AMCompat.contains(compound, "MineStack")){
            this.lastGivenStack = com.github.alexthe666.alexsmobs.misc.AMCompat.loadItem(this.level().registryAccess(), AMCompat.getCompound(compound, "MineStack"));
        }
    }
    ...
    private float calculateDistanceToFloor() {
        ...
        while (!level().getBlockState(floor).isFaceSturdy(level(), floor, Direction.UP) && floor.getY() > AMCompat.minBuildHeight(level())) {
            floor = floor.below();
        }
    ...
    protected void populateDefaultEquipmentSlots(RandomSource p_218949_, DifficultyInstance p_218950_) {
        ...
        // 1.21.5 removed the getEquipmentDropChance override point; configure the chance directly.
    ...
    *///?} else {
    ...
    private boolean isActuallyInAWall() {
        final float f = com.github.alexthe666.alexsmobs.misc.AMCompat.width(this.getDimensions(this.getPose())) * 0.1F;
            ...
            if (minePretendPos != null && minePretendStartState != null) {
                ...
                if (distSqr < 6.5F) {
                    ...
                    if(EntityUnderminer.this.getNavigation().isDone()) {
                        ...
                        if (mineTime % 10 == 0) {
                            // The four-argument getSoundType is a Forge IForgeBlock extension; vanilla
                            // only has the context-free BlockStateBase#getSoundType().
                            //? if !fabric
