...
public class EntityFrilledShark extends WaterAnimal implements IAnimatedEntity, Bucketable {
    ...
    *///?} else {
    ...
    public void tick() {
        ...
        // The target is read ONCE into a local. Upstream called getTarget() four times across the
        // damage call, and the damage call itself can clear it — a target that dies from the bite,
        // or one whose own hurt handler makes this shark re-evaluate — so the two reads after
        // AMCompat.hurt could come back null and the addEffect NPE'd, killing the whole server tick
        // ("getting close to a frilled shark crashes", report #36). Nothing here needs the live
        // value: it is the entity that was just bitten that gets the exsanguination.
        ...
        if (!this.level().isClientSide() && target != null && this.getAnimation() == ANIMATION_ATTACK && this.getAnimationTick() == 12) {
            ...
            if (AMCompat.hurt(target, this.damageSources().mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue())){
                ...
                if(random.nextInt(15) == 0 && target instanceof Squid){
                    AMCompat.spawnAtLocation(this, AMItemRegistry.SERRATED_SHARK_TOOTH.get());
                }
            }
            ...
            if (EntityFrilledShark.this.distanceTo(target) < 10) {
                if (EntityFrilledShark.this.distanceTo(target) < 1.9D) {
                    ...
                    if (target instanceof Squid) {
                        ...
                        //?}
                        // SLICE: Squid#setMovementVector removed in 1.21.2 — squid-suck pull dropped; teeth-glint particle kept.
