    ...
    *///?} else {
    ...
    private boolean teleport(double x, double y, double z) {
        ...
        if (flag && !blockstate.getFluidState().is(FluidTags.WATER)) {
            this.playSound(SoundEvents.ENDERMAN_TELEPORT, 1.0F, 1.0F);
            // EventBus 7 (Forge 1.21.6) took isCanceled() off the event, so the factory helper can no
            // longer report the veto — post the event directly and read post()'s boolean instead.
            //? if forge && >=1.21.6 {
            /*net.minecraftforge.event.entity.EntityTeleportEvent.EnderEntity event = new net.minecraftforge.event.entity.EntityTeleportEvent.EnderEntity(this, x, y, z);
            if (net.minecraftforge.event.entity.EntityTeleportEvent.EnderEntity.BUS.post(event)) return false;
            *///?} elif fabric {
            /*// Fabric has no ender-teleport event, so the destination cannot be vetoed or moved.
            *///?} else {
