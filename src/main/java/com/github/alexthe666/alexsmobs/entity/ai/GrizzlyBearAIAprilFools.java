    ...
    public boolean canUse() {
        if(!bear.isBaby() && AlexsMobs.isAprilFools() && runDelay-- <= 0 && bear.getRandom().nextInt(30) == 0){
            ...
            Player nearestPlayer = bear.level().getNearestPlayer(bear.getX(), bear.getY(), bear.getZ(), maxDistance, entity -> {
                return bear.hasLineOfSight(entity) &&(!(entity instanceof Player) || !((Player) entity).hasEffect(AMCompat.effect(AMEffectRegistry.POWER_DOWN.get())));
            });
    ...
    public void tick() {
        ...
        if(dist <= 6 && bear.hasLineOfSight(target)){
            ...
            if(bear.getAprilFoolsFlag() == 5){
                ...
                if(powerOutTimer == 0){
                    target.addEffect(new MobEffectInstance(AMCompat.effect(AMEffectRegistry.POWER_DOWN.get()), 2 * (maxMusicBoxTime + 100), 0, false, false, true));
                }
                powerOutTimer++;
                if (powerOutTimer >= 60) {
                ...
                if (musicBoxTimer >= maxMusicBoxTime) {
                    if(bear.getAprilFoolsFlag() != 5){
                        ...
                        bear.gameEvent(AMPlatform.ENTITY_ACTION);
