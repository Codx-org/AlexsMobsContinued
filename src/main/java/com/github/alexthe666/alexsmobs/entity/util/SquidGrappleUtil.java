...
public class SquidGrappleUtil {
    ...
    public static int onFireHook(LivingEntity entity, UUID newHookUUID) {
        ...
        if(tag.contains(indexStr)){
            EntitySquidGrapple hook = getHookEntity(entity.level(), AMCompat.getUUID(tag, indexStr));
            if(hook != null && !hook.isRemoved()){
                hook.setWithdrawing(true);
            }
        }
        AMCompat.putUUID(tag, indexStr, newHookUUID);
    ...
    public static int getFirstAvailableHookIndex(LivingEntity entity){
        ...
        if(i < 4){
            ...
            CompoundTag tag = CitadelEntityData.getOrCreateCitadelTag(entity);
            int j = AMCompat.getInt(tag, LAST_REPLACED_HOOK);
    ...
    public static int getAnyNullHooks(LivingEntity entity) {
        CompoundTag tag = CitadelEntityData.getOrCreateCitadelTag(entity);
        if (!tag.contains(HOOK_1) || getHookEntity(entity.level(), AMCompat.getUUID(tag, HOOK_1)) == null) {
            return 0;
        }
        if (!tag.contains(HOOK_2) || getHookEntity(entity.level(), AMCompat.getUUID(tag, HOOK_2)) == null) {
            return 1;
        }
        if (!tag.contains(HOOK_3) || getHookEntity(entity.level(), AMCompat.getUUID(tag, HOOK_3)) == null) {
            return 2;
        }
        if (!tag.contains(HOOK_4) || getHookEntity(entity.level(), AMCompat.getUUID(tag, HOOK_4)) == null) {
            return 3;
        }
        return -1;
    }
