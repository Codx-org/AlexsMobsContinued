            ...
            if (target == kangaroo.getLastHurtByMob()) {
                ...
                if (waterPos == null || !kangaroo.level().getFluidState(waterPos).is(FluidTags.WATER)) {
                    ...
                    if(waterTimeout < 1400){
                        dontSuper = true;
                        amCheckAndPerformAttack(target, kangaroo.distanceToSqr(target));
                    }
...
//?}
    ...
    protected void amCheckAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
        double d0 = com.github.alexthe666.alexsmobs.misc.AMPlatform.attackReachSqr(this.mob, enemy) + 5D;
        if (distToEnemySqr <= d0) {
            if(kangaroo.isInWater()){
                ...
                AMCompat.knockback(enemy, 1F, enemy.getX() - kangaroo.getX(), enemy.getZ() - kangaroo.getZ());

            }
            this.resetAttackCooldown();
            if(kangaroo.getAnimation() == IAnimatedEntity.NO_ANIMATION){
                // Not upstream (#59): a kangaroo holding a spear winds the weapon up over its
                // shoulder and thrusts, rather than throwing the same bare-fisted hook at everything.
                if(kangaroo.getMainHandItem().is(com.github.alexthe666.alexsmobs.misc.AMTagRegistry.KANGAROO_SPEARS)){
                    kangaroo.setAnimation(kangaroo.isLeftHanded() ? EntityKangaroo.ANIMATION_STAB_L : EntityKangaroo.ANIMATION_STAB_R);
                }else if(kangaroo.getRandom().nextBoolean()){
