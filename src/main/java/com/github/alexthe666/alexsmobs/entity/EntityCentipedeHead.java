    ...
    *///?} else {
    ...
    public void tick() {
        ...
        *///?} else {
        ...
        //?}
        /* yBodyRot is the ONLY thing the head's renderer turns by (RenderCentipedeHead uses the
         * interpolated yBodyRot; the segments derive their own yaw geometrically from the getYRot()
         * ring buffer below), so this chase is what the player sees the head pointing at.
         *
         * Upstream wrote it as Mth.clamp(getYRot(), yBodyRot - 2, yBodyRot + 2), which has two
         * faults that together read as "the head never rotates, it just faces south":
         *   1. No wrapDegrees on the delta. getYRot() is normalised to [-180,180], so the moment the
         *      mob turns across that boundary the clamp chases the LONG way — up to 360 degrees of
         *      wrong-way rotation, dragging yBodyRot back through 0 (= south) every single time.
         *   2. 2 deg/tick is 40 deg/s, while MoveControl can swing getYRot() by up to 90 deg in ONE
         *      tick and RandomStrollGoal re-targets every couple of seconds. The chase therefore
         *      never converged at all; it random-walked, and fault 1 biased that walk toward 0.
         * Wrapping the delta and raising the limit makes it track. The head still visibly lags the
         * body (a 180 deg turn takes 18 ticks), which is the intended stiff-segment look, and the
         * segment trail is untouched because it never reads yBodyRot. */
        this.yBodyRot += Mth.clamp(Mth.wrapDegrees(this.getYRot() - this.yBodyRot), -10.0F, 10.0F);
