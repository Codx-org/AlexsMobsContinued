...
public class EntitySeal extends Animal implements ISemiAquatic, IHerdPanic, ITargetsDroppedItems {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(10, new TemptGoal(this, 1.1D, AMCompat.ingredientOfTags(AMTagRegistry.SEAL_BREEDABLES, AMTagRegistry.SEAL_OFFERINGS), false));
    ...
    *///?} else {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            // The two isSeekingTreasure() clauses are part of #83: SealAIBask is priority 0 and holds
            // Flag.MOVE, so a basking seal outranks the dive goal outright. Upstream got away with
            // that only because the dive goal declared NO flags at all and could clear the flag from
            // its own tick(); now that it holds MOVE (so the wander goals stop dragging it off the
            // dig site), basking has to be suppressed here instead.
            if (isBasking()) {
                if (this.getLastHurtByMob() != null || isInLove() || revengeCooldown > 0 || this.isInWaterOrBubble() || this.getTarget() != null || isSeekingTreasure() || baskingTimer > 1000 && this.getRandom().nextInt(100) == 0) {
                    this.setBasking(false);
                }
            } else {
                if (this.getTarget() == null && !isInLove() && this.getLastHurtByMob() == null && revengeCooldown == 0 && !isBasking() && baskingTimer == 0 && !isSeekingTreasure() && this.getRandom().nextInt(15) == 0) {
            ...
            // UPSTREAM FIX (#83): swimTimer had no bounds, so it is not a "how long since I last
            // swapped element" counter at all — it is the whole time the seal has spent on one side
            // MINUS the other. A seal that basked for twenty minutes reaches about -25000 and then,
            // once it finally gets in, needs that same twenty minutes back before shouldLeaveWater()
            // can fire; the reverse strands one in the sea. Clamping just past each threshold keeps
            // the intended ~80 s hysteresis and nothing else.
            swimTimer = Mth.clamp(swimTimer, -1200, 800);
        }
    ...
    *///?} else {
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        if(feederUUID != null){
            AMCompat.putUUID(compound, "FeederUUID", feederUUID);
        }
    }
    ...
    /**
     * True while this seal owes a player a treasure and has not dug it up yet — i.e. it has been fed
     * its three fish and SealAIDiveForItems wants it on the seafloor. See #83.
     */
    public boolean isSeekingTreasure() {
        // feederUUID is persisted and is only cleared on delivery, so the feeder being present is
        // part of the question — otherwise a seal whose feeder logged out would never bask again.
        return this.feederUUID != null && this.getMainHandItem().isEmpty()
                && this.level().getPlayerByUUID(this.feederUUID) != null;
    }

    /**
     * True while this seal has been attacked recently and should be making for open water. See #83.
     */
    public boolean isFleeingToWater() {
        return this.revengeCooldown > 0 || this.getLastHurtByMob() != null;
    }

    // UPSTREAM FIX (#83). Both of the seal's water gates were driven by swimTimer and nothing else,
    // so the two situations where a seal most obviously belongs in the water could not move it:
    //
    //  * FED ITS THREE FISH. SealAIDiveForItems can only pick a dig site it can see (+-7 blocks of
    //    water, or of the seal itself once submerged), and nothing else ever pushed the seal in — so
    //    a seal fed on a beach with swimTimer anywhere above -1000 (i.e. for the ~80 s after it last
    //    hauled out, and forever if it had just been basking) simply sat there. AnimalAIFindWater
    //    searches 32 blocks and is priority 3, so answering "yes" here is the whole trip.
    //  * ATTACKED. A hurt seal runs AnimalAIHerdPanic, which flees via LandRandomPos - onto LAND,
    //    away from the one place a seal is safe. Worse, a seal that had been swimming (swimTimer
    //    > 600) and was hit at the water's edge answered shouldLeaveWater() = true and hauled out
    //    straight past its attacker. See also canPanic().
    @Override
    public boolean shouldEnterWater() {
        if (isSeekingTreasure() || isFleeingToWater()) {
            return true;
        }
    ...
    // UPSTREAM FIX (#83), the other half of shouldEnterWater(): AnimalAIHerdPanic flees to a
    // LandRandomPos, so a seal that HAD made it into the water was immediately pathed back out onto
    // the beach — right past whoever was hitting it. A seal in the water has already escaped;
    // panicking is a land behaviour.
    @Override
    public boolean canPanic() {
        return !isBasking() && !this.isInWaterOrBubble();
    }
