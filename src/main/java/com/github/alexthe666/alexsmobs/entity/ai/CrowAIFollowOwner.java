...
public class CrowAIFollowOwner extends Goal {
    ...
    public void tick() {
        ...
        if (!this.crow.isLeashed() && !this.crow.isPassenger()) {
            ...
            if (!crow.aiItemFlag) {
                ...
                if(circlingTime > maxCircleTime && crow.getRidingCrows(owner) < 2 && crow.canBoardOwner(owner)){
                    ...
                    if(crow.distanceTo(owner) < 2){
                        AMCompat.startRiding(crow, owner, true);
    ...
    private boolean isTeleportFriendlyBlock(BlockPos p_226329_1_) {
        BlockPathTypes lvt_2_1_ = AMCompat.pathTypeStatic(this.crow, p_226329_1_);
