...
public class EntityCachalotWhale extends Animal implements IMultipartOwner {
                            ...
                            if (this.isCharging()) {
                                if (this.distanceTo(target) < this.getBbWidth() && chargeProgress > 4) {
                                    ...
                                    if (target.getVehicle() instanceof final Boat boat) {
                                        for (int i = 0; i < 3; ++i) {
                                            //? if >=1.21.2 {
                                            /*// SLICE: Boat#getVariant removed in 1.21.2 — drop oak planks regardless of boat type.
                                            AMCompat.spawnAtLocation(this, Items.OAK_PLANKS);
                                            *///?} else {
    ...
    *///?} else {
    ...
    public Vec3 getDismountLocationForPassenger(LivingEntity dismount) {
        ...
        while(!level().isEmptyBlock(pos) && !level().isWaterAt(pos) && pos.getY() < AMCompat.maxBuildHeight(level())){
            pos = pos.above();
        }
    ...
    class AIBreathe extends Goal {
        ...
        private boolean canBreatheAt(LevelReader p_205140_1_, BlockPos p_205140_2_) {
            ...
            return (p_205140_1_.getFluidState(p_205140_2_).isEmpty() || lvt_3_1_.is(Blocks.BUBBLE_COLUMN)) && AMCompat.isPathfindable(lvt_3_1_, p_205140_1_, p_205140_2_, PathComputationType.LAND);
        }
    }
}
