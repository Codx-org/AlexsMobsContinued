...
public class EntityTiger extends Animal implements ICustomCollisions, IAnimatedEntity, NeutralMob, ITargetsDroppedItems {
    ...
    private static final Predicate<LivingEntity> NO_BLESSING_EFFECT = (mob) -> {
        return !mob.hasEffect(AMCompat.effect(AMEffectRegistry.TIGERS_BLESSING.get()));
    };
    ...
    //? if >=1.21.11 {
    /*private net.minecraft.world.entity.EntityReference<net.minecraft.world.entity.LivingEntity> lastHurtBy;
    *///?} else {
    ...
    *///?} else {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(4, new NearestAttackableTargetGoal(this, LivingEntity.class, 220, false, false, AMCompat.selector(AMEntityRegistry.buildPredicateFromTag(AMTagRegistry.TIGER_TARGETS))) {
    ...
    // 1.21.11 swapped NeutralMob's remaining-ticks pair for an absolute end time AND dropped the
    // countdown that used to live in updatePersistentAnger. The existing storage above stays the
    // source of truth (it is synched, and the mod's own renderers read it client-side), the end
    // time is derived from it, and customServerAiStep does the decrement vanilla no longer does.
    //? if >=1.21.11 {
    /*public long getPersistentAngerEndTime() {
        int remaining = this.getRemainingPersistentAngerTime();
        return remaining <= 0 ? -1L : this.level().getGameTime() + remaining;
    }

    public void setPersistentAngerEndTime(long endTime) {
        this.setRemainingPersistentAngerTime(endTime <= 0L ? 0 : (int) Math.max(0L, endTime - this.level().getGameTime()));
    }
    ...
    public void setPersistentAngerTarget(@Nullable net.minecraft.world.entity.EntityReference<net.minecraft.world.entity.LivingEntity> target) {
    ...
    *///?} else {
    ...
    protected void customServerAiStep() {
        if (!this.level().isClientSide()) {
            //? if >=1.21.11
            //if (this.getRemainingPersistentAngerTime() > 0) this.setRemainingPersistentAngerTime(this.getRemainingPersistentAngerTime() - 1);
    ...
    // Entity#collide is PRIVATE in vanilla; Forge patches it to be overridable. A Fabric access
    // widener cannot substitute — the AW spec makes a private method that is made accessible also
    // final, precisely to preserve invokespecial semantics — so the override cannot exist on Fabric
    // and the gate below stays. The behaviour is not lost: FabricEntityMixin#alexsmobs$customCollisions
    // injects at Entity#collide HEAD and delegates for any ICustomCollisions, this class included.
    //? if !fabric {
    ...
    public boolean canTargetItem(ItemStack stack) {
        return AMCompat.isEdible(stack.getItem()) && AMCompat.getFoodProperties(stack.getItem()) != null && AMCompat.isMeat(stack.getItem()) && stack.getItem() != Items.ROTTEN_FLESH;
    }
    ...
    public void onGetItem(ItemEntity e) {
        ...
        if (AMCompat.isEdible(stack.getItem()) && AMCompat.getFoodProperties(stack.getItem()) != null && AMCompat.isMeat(stack.getItem()) && stack.getItem() != Items.ROTTEN_FLESH) {
    ...
    static class TigerNodeEvaluator extends WalkNodeEvaluator {
        //? if >=1.20.5 {
        /*// 1.20.5 dropped the protected evaluateBlockPathType hook; per-block classification
        // now runs through getPathType(PathfindingContext, x, y, z).
        public BlockPathTypes getPathType(net.minecraft.world.level.pathfinder.PathfindingContext context, int x, int y, int z) {
            BlockPathTypes typeIn = super.getPathType(context, x, y, z);
            BlockPos pos = new BlockPos(x, y, z);
            return typeIn == BlockPathTypes.LEAVES || context.getBlockState(pos).getBlock() == Blocks.BAMBOO ? BlockPathTypes.OPEN : typeIn;
        }
        *///?} else {
    ...
    class AttackPlayerGoal extends NearestAttackableTargetGoal<Player> {
        ...
        public AttackPlayerGoal() {
            super(EntityTiger.this, Player.class, 100, false, true, AMCompat.selector(NO_BLESSING_EFFECT));
        }
