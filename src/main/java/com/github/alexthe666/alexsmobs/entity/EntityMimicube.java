...
public class EntityMimicube extends Monster implements RangedAttackMob {
    ...
    *///?} else {
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        ...
        // A custom bow gets to post-process the arrow it just fired. NeoForge's hook also takes the
        // ammo stack so the bow can inspect it; Forge dropped the hook outright in 1.21.
        //? if forge && <1.21 {
        ...
        *///?}
        // 1.21 also hands the bow itself over, so a custom bow can read its own components.
        //? if neoforge && >=1.21 {
        /*if (this.getMainHandItem().getItem() instanceof net.minecraft.world.item.BowItem)
    ...
    protected AbstractArrow fireArrow(ItemStack arrowStack, float distanceFactor) {
        // 1.21 threads the firing weapon through so the arrow can read its enchantments.
        //? if >=1.21 {
    ...
    public void tick() {
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            if (this.entityData.get(ATTACK_TICK) == 2 && this.getTarget() != null && this.distanceTo(this.getTarget()) < 2.3D) {
                //? if >=1.21.2
                /*net.minecraft.server.level.ServerLevel amLevel = (net.minecraft.server.level.ServerLevel) this.level();*/
