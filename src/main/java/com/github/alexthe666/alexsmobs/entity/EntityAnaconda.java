...
public class EntityAnaconda extends Animal implements ISemiAquatic {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.25D, AMCompat.ingredientOf(AMTagRegistry.ANACONDA_FOODSTUFFS), false));
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal(this, LivingEntity.class, 200, false, false, AMCompat.selector(AMEntityRegistry.buildPredicateFromTag(AMTagRegistry.ANACONDA_TARGETS))));
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        feedings = AMCompat.getInt(compound, "Feedings");
        this.setSheddingTime(AMCompat.getInt(compound, "ShedTime"));
        this.setYellow(AMCompat.getBoolean(compound, "Yellow"));
        shedCooldown = AMCompat.getInt(compound, "ShedCooldown");
        passiveFor = AMCompat.getInt(compound, "PassiveFor");
    }
...
//                float headRot = Mth.wrapDegrees(this.getYRot());
    ...
    public ItemEntity spawnItemAtOffset(ItemStack stack, float f, float f1) {
        if (stack.isEmpty()) {
            ...
            // captureDrops is Forge's drop-interception buffer; nothing equivalent exists on Fabric,
            // so the item is always spawned directly there.
            //? if !fabric {
