...
public class EntitySkreecher extends Monster {
    ...
    *///?} else {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if(this.isClinging()){
                ...
                if (technicalDistToCeiling > MAX_DIST_TO_CEILING || !isAlive() || clingCooldown > 0 || com.github.alexthe666.alexsmobs.misc.AMPlatform.isInAnyFluid(this)) {
                    this.setClinging(false);
                }
            ...
            if(clapTick >= 100){
                if(!hasAttemptedWardenSpawning && AMConfig.skreechersSummonWarden){
                    ...
                    if(!this.level().isClientSide() && getNearbyWardens().isEmpty() && holder.is(AMTagRegistry.SKREECHERS_CAN_SPAWN_WARDENS)){
                        ...
                        //? if >=1.20.5 {
                        /*warden.finalizeSpawn((ServerLevel)level(), AMCompat.difficultyAt(level(), this.blockPosition()), MobSpawnType.TRIGGERED, (SpawnGroupData)null);
                        *///?} else {
                        warden.finalizeSpawn((ServerLevel)level(), AMCompat.difficultyAt(level(), this.blockPosition()), MobSpawnType.TRIGGERED, (SpawnGroupData)null, (CompoundTag)null);
                        //?}
    ...
    public void travel(Vec3 travelVector) {
        if (this.isEffectiveAi() && this.isClinging() && !com.github.alexthe666.alexsmobs.misc.AMPlatform.isInAnyFluid(this)) {
    ...
    public BlockPos getCeilingOf(BlockPos usPos){
        while (!level().getBlockState(usPos).isFaceSturdy(level(), usPos, Direction.DOWN) && usPos.getY() < AMCompat.maxBuildHeight(level())){
            usPos = usPos.above();
        }
        return usPos;
    }
