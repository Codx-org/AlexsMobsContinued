    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new MeleeAttackGoal(this, 1.5D, true) {
            protected double getAttackReachSqr(LivingEntity attackTarget) {
                return com.github.alexthe666.alexsmobs.misc.AMPlatform.attackReachSqr(this.mob, attackTarget) + 1.5D;
            }

            // 1.20.2 stopped consulting getAttackReachSqr (canPerformAttack now uses vanilla's
            // narrower isWithinMeleeAttackRange), so re-apply this goal's wider reach here.
//? if >=1.20.2 {
...
*///?}
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.0D, AMCompat.ingredientOfTags(AMTagRegistry.GELADA_MONKEY_BREEDABLES, AMTagRegistry.GELADA_MONKEY_LAND_CLEARING_FOODS), false));
        ...
        this.targetSelector.addGoal(2, leaderFightGoal = new NearestAttackableTargetGoal<EntityGeladaMonkey>(this, EntityGeladaMonkey.class, 70, false, false, com.github.alexthe666.alexsmobs.misc.AMCompat.selector((monkey) -> {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (this.getTarget() != null && (this.getAnimation() == ANIMATION_SWIPE_L || this.getAnimation() == ANIMATION_SWIPE_R) && this.getAnimationTick() == 7 && this.hasLineOfSight(this.getTarget()) && this.distanceTo(this.getTarget()) < this.getBbHeight() + this.getTarget().getBbHeight() + 1) {
                AMCompat.knockback(getTarget(), 0.4F, getTarget().getX() - this.getX(), getTarget().getZ() - this.getZ());
