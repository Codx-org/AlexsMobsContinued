...
public class EntityBunfungus extends PathfinderMob implements IAnimatedEntity {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, Mob.class, 5, false, false, AMCompat.selector((mob) -> {
            return mob instanceof Enemy && !(mob instanceof Creeper) && !(AMCompat.isAquatic(mob) && mob.isInWaterOrBubble()) && !mob.getType().builtInRegistryHolder().is(AMTagRegistry.BUNFUNGUS_IGNORES);
        })));
    }
                ...
                if (this.getAnimationTick() == 5) {
                    if (dist < 3.5D && this.getAnimation() == ANIMATION_BELLY) {
                        ...
                        for (final LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(2.0D))) {
                            if ((entity == target || entity instanceof Monster) && !entity.getType().builtInRegistryHolder().is(AMTagRegistry.BUNFUNGUS_IGNORE_AOE_ATTACKS)) {
                                flag = true;
                                AMCompat.knockback(entity, 0.2F, entity.getX() - this.getX(), entity.getZ() - this.getZ());
                                entity.hurt(this.damageSources().mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue());
                            }
                        }
                    }
