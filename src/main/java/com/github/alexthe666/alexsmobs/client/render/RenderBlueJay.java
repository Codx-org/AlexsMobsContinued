...
public class RenderBlueJay extends MobRenderer<EntityBlueJay, ModelBlueJay> {
    private static final ResourceLocation TEXTURE = AMCompat.rl("alexsmobs:textures/entity/blue_jay.png");
    private static final ResourceLocation TEXTURE_SHINY = AMCompat.rl("alexsmobs:textures/entity/blue_jay_shiny.png");
    ...
    protected void scale(EntityBlueJay mob, PoseStack matrixStackIn, float partialTicks) {
        ...
        if(mob.isPassenger() && mob.getVehicle() != null) {
            if (mob.getVehicle() instanceof EntityRaccoon entityRaccoon) {
                if (com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.rendererModel(entityRaccoon) instanceof ModelRaccoon raccoonModel) {
