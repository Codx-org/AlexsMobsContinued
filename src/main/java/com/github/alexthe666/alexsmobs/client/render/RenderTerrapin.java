    ...
    static class TurtleOverlayLayer extends RenderLayer<EntityTerrapin, ModelTerrapin> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource buffer, int packedLightIn, EntityTerrapin turtle, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            if(turtle.getTurtleType() == TerrapinTypes.OVERLAY && !turtle.isKoopa()){
                ResourceLocation tex = layer == 0 ? parent.getTextureLocation(turtle) : layer == 1 ? SHELL_TEXTURES[turtle.getShellType() % SHELL_TEXTURES.length] : SKIN_PATTERN_TEXTURES[turtle.getSkinType() % SKIN_PATTERN_TEXTURES.length];
                int color = layer == 0 ? turtle.getTurtleColor() : layer == 1 ? turtle.getShellColor() : turtle.getSkinColor();
                float r = (float) (color >> 16 & 255) / 255.0F;
                float g = (float) (color >> 8 & 255) / 255.0F;
                float b = (float) (color & 255) / 255.0F;
                //? if >=1.21.2 {
                /*renderColoredModel(getParentModel(), tex, matrixStackIn, buffer, packedLightIn, AMRenderCompat.packColor(r, g, b, 1.0F));
                *///?} elif >=1.21 {
                /*renderColoredCutoutModel(getParentModel(), tex, matrixStackIn, buffer, packedLightIn, turtle, AMRenderCompat.packColor(r, g, b, 1.0F));
                *///?} else {
