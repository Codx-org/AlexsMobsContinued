...
public class LayerElephantItem extends RenderLayer<EntityElephant, ModelElephant> {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityElephant entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        //? if >=1.21.4 {
        /*if (false) { // 1.21.4 removed ItemRenderer#getModel; the gui3d layout tweak is dropped (cosmetic)
        *///?} elif >=1.21.2 {
        /*if(Minecraft.getInstance().getItemRenderer().getModel(itemstack, entitylivingbaseIn.level(), entitylivingbaseIn, 0).isGui3d()){
        *///?} else {
        ...
        com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.renderItemInHand(renderer, entitylivingbaseIn, itemstack, ItemDisplayContext.GROUND, false, matrixStackIn, bufferIn, packedLightIn);
