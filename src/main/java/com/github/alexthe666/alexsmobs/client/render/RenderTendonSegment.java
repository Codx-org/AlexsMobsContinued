...
public class RenderTendonSegment extends EntityRenderer<EntityTendonSegment> {
    ...
    public void render(EntityTendonSegment entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int light) {
        ...
        if (fromEntity != null) {
            ...
            Vec3 from = distVec;
            // The neck-segment emission, so it can be replayed once per render type when the whip is
            // enchanted. It owns every mutable local it touches — the glint pass and the body pass must
            // each walk the chain from the start. See AMRenderTypes.renderMerged for why two passes.
            java.util.function.Consumer<VertexConsumer> neckGeometry = neckConsumer -> {
                int segmentCount = 0;
            ...
            if(entity.hasClaw() || entity.isRetracting()){
                java.util.function.Consumer<VertexConsumer> clawGeometry = clawConsumer -> {
                    poseStack.pushPose();
    ...
    private Vec3 getPositionOfPriorMob(EntityTendonSegment segment, Entity mob, float partialTicks){
        ...
        if(mob instanceof Player && segment.isCreator(mob)){
            ...
            if ((this.entityRenderDispatcher.options == null || this.entityRenderDispatcher.options.getCameraType().isFirstPerson()) && player == Minecraft.getInstance().player) {
                ...
                //? if >=26
                /*Vec3 vec3 = this.entityRenderDispatcher.camera.getNearPlane((float) this.entityRenderDispatcher.options.fov().get().intValue()).getPointOnPlane((float) i * 0.6F, -1);*/
                //? if <26
