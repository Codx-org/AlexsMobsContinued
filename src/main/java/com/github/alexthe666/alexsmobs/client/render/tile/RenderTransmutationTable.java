...
public class RenderTransmutationTable<T extends TileEntityTransmutationTable> implements BlockEntityRenderer<T> {
    ...
    @Override
    //? if >=1.21.5 {
    /*public void render(T tileEntityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, net.minecraft.world.phys.Vec3 amCamPos) {
    *///?} else {
    ...
    //?}
        ...
        AMRenderTypes.renderStaticOverlay(bufferIn, AMRenderTypes.STATIC_PORTAL, RenderType.entityCutoutNoCull(OVERLAY),
                staticyOverlay -> OVERLAY_MODEL.renderToBuffer(matrixStackIn, staticyOverlay, combinedLightIn, OverlayTexture.NO_OVERLAY, 1, 1, 1, 1));
    ...
    private static void vertex(VertexConsumer p_114090_, Matrix4f p_114091_, Matrix3f p_114092_, int p_114093_, float p_114094_, float p_114095_, int p_114096_, int p_114097_) {
        com.github.alexthe666.alexsmobs.client.render.AMVertex.normal(p_114090_.vertex(p_114091_, p_114094_, p_114095_, 0.0F).color(255, 255, 255, 100).uv((float) p_114096_, (float) p_114097_).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_114093_), p_114092_, 0.0F, 1.0F, 0.0F).endVertex();
    }
