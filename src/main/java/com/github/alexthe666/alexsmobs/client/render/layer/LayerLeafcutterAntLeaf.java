...
public class LayerLeafcutterAntLeaf extends RenderLayer<EntityLeafcutterAnt, AdvancedEntityModel<EntityLeafcutterAnt>> {
    ...
    private static final ResourceLocation TEXTURE_0 = AMCompat.rl("alexsmobs:textures/entity/leafcutter_ant_leaf_0.png");
    private static final ResourceLocation TEXTURE_1 = AMCompat.rl("alexsmobs:textures/entity/leafcutter_ant_leaf_1.png");
    private static final ResourceLocation TEXTURE_2 = AMCompat.rl("alexsmobs:textures/entity/leafcutter_ant_leaf_2.png");
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityLeafcutterAnt entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn.hasLeaf() && !entitylivingbaseIn.isQueen() && this.getParentModel() instanceof ModelLeafcutterAnt) {
            ...
            //? if (>=1.21.4 || fabric) {
            /*// 1.21.4 removed Minecraft#getItemColors; the jungle-leaves item tint delegates to the block
            // colour anyway, so read the default foliage colour straight from BlockColors (equivalent).
            // Fabric takes this arm on every version: below 1.21.4 the field is still there but the
            // getter is a Forge patch, and this is the same colour by a route vanilla already exposes.
            int leafColor = com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.blockTintColor(net.minecraft.world.level.block.Blocks.JUNGLE_LEAVES.defaultBlockState(), 0);
            *///?} else {
