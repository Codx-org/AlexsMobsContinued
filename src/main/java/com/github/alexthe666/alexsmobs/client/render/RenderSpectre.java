...
public class RenderSpectre extends MobRenderer<EntitySpectre, ModelSpectre> {
     ...
     * This one had no {@code render} of its own at all and leaned entirely on vanilla's, so the
     * two-line body below is that method, reproduced against the compat model's own eight-float
     * {@code renderToBuffer} (vanilla's is {@code final} from 1.21.2 and walks the empty root).
     */
