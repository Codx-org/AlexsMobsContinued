...
public class ModelMurmurHead extends AdvancedEntityModel<EntityMurmurHead> {
    ...
    public void setupAnim(EntityMurmurHead entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if(ageInTicks > 5F){
            ...
            // Upstream's `Mth.lerp(partialTicks, entity.walkDistO, entity.walkDist)`, restored — the
            // pair is now the entity's own, because 1.21.2 deleted Entity's. Not walkAnimation:
            // that is a different quantity and swung the hair about seven times too fast (#68).
            f1 += Mth.sin(Mth.lerp(partialTicks, entity.amWalkDistO, entity.amWalkDist) * 6.0F) * 32.0F * 1F;
