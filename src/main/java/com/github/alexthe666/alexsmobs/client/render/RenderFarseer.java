...
public class RenderFarseer extends MobRenderer<EntityFarseer, ModelFarseer> {
    ...
    // Static noise pre-baked into each portal frame (4 noise variants per frame), generated by
    // scripts/bake_static_textures.py. Only drawn on >=1.21.5, where the two-pass EQUAL-depth
    // static overlay no longer exists — see AMRenderTypes.renderStaticMasked (#53).
    ...
    public void render(EntityFarseer entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        *///?} elif fabric {
        ...
        if (false)
        *///?} else {
        if (com.github.alexthe666.alexsmobs.misc.AMPlatform.postCancelled(new net.minecraftforge.client.event.RenderLivingEvent.Pre<EntityFarseer, ModelFarseer>(entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn)))
        //?}
            return;
        LivingEntity laserTarget = entityIn.getLaserTarget();
        float faceCameraAmount = entityIn.getFacingCameraAmount(partialTicks);
        Quaternionf camera = com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.cameraOrientation(this.entityRenderDispatcher);
        ...
        *///?} else {
        ...
        if(entityIn.getAnimation() == EntityFarseer.ANIMATION_EMERGE){
            ...
            AMRenderTypes.renderStaticMasked(bufferIn, AMRenderTypes.STATIC_PORTAL, PORTAL_TEXTURES[portalTexture], PORTAL_STATIC_TEXTURES[portalTexture][noiseVariant], portalStatic -> {
    ...
    private void renderFarseerModel(PoseStack matrixStackIn, MultiBufferSource source, RenderType defRenderType, float partialTicks, int packedLightIn, int overlayColors, float alphaIn, EntityFarseer entityIn) {
        // The eye and scars masks are pure black with an alpha cut-out — everything visible about
        // them was the STATIC_ENTITY pass drawn over them at EQUAL depth, so on >=1.21.5 they need
        // the pre-baked static, not a bare shaped pass (which is a black eye and black scars, #90).
        final int noise = Math.floorMod(entityIn.tickCount / 2, EYE_STATIC_TEXTURES.length);
        if(entityIn.hasLaser()){
            AMRenderTypes.renderStaticMasked(source, AMRenderTypes.STATIC_ENTITY, TEXTURE_EYE, EYE_STATIC_TEXTURES[noise],
                    staticyInsides -> EYE_MODEL.renderToBuffer(matrixStackIn, staticyInsides, packedLightIn, NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1F));
        }
        ...
        if(hurt > 0){
            ...
            AMRenderTypes.renderStaticMasked(source, AMRenderTypes.STATIC_ENTITY, TEXTURE_SCARS, SCARS_STATIC_TEXTURES[noise],
    ...
    private static void laserOriginVertex(VertexConsumer p_114220_, Matrix4f p_114221_, Matrix3f p_114092_, int p_114222_, float xOffset, float yOffset) {
        com.github.alexthe666.alexsmobs.client.render.AMVertex.normal(p_114220_.vertex(p_114221_, 0.0F, 0.0F, 0.0F).color(255, 255, 255, 255).uv(xOffset + 0.5F, yOffset).overlayCoords(NO_OVERLAY).uv2(240), p_114092_, 0.0F, 1.0F, 0.0F).endVertex();
    }
    ...
    private static void laserLeftCornerVertex(VertexConsumer p_114215_, Matrix4f p_114216_, Matrix3f p_114092_, float p_114217_, float p_114218_, float xOffset, float yOffset) {
        com.github.alexthe666.alexsmobs.client.render.AMVertex.normal(p_114215_.vertex(p_114216_, -HALF_SQRT_3 * p_114218_, p_114217_, 0).color(255, 255, 255, 0).uv(xOffset, yOffset + 1).overlayCoords(NO_OVERLAY).uv2(240), p_114092_, 0.0F, -1.0F, 0.0F).endVertex();
    }
    ...
    private static void laserRightCornerVertex(VertexConsumer p_114224_, Matrix4f p_114225_, Matrix3f p_114092_, float p_114226_, float p_114227_, float xOffset, float yOffset) {
        com.github.alexthe666.alexsmobs.client.render.AMVertex.normal(p_114224_.vertex(p_114225_, HALF_SQRT_3 * p_114227_, p_114226_, 0).color(255, 255, 255, 0).uv(xOffset + 1, yOffset + 1).overlayCoords(NO_OVERLAY).uv2(240), p_114092_, 0.0F, -1.0F, 0.0F).endVertex();
    }
    ...
    private static void portalVertex(VertexConsumer p_114090_, Matrix4f p_114091_, Matrix3f p_114092_, int p_114093_, float p_114094_, int p_114095_, int p_114096_, int p_114097_, float alpha) {
        com.github.alexthe666.alexsmobs.client.render.AMVertex.normal(p_114090_.vertex(p_114091_, p_114094_ - 0.5F, (float)p_114095_ - 0.25F, 0.0F).color(1F, 1F, 1F,  alpha).uv((float)p_114096_, (float)p_114097_).overlayCoords(NO_OVERLAY).uv2(240), p_114092_, 0.0F, -1.0F, 0.0F).endVertex();
    }
    @Override
    //? if >=1.20.5 {
