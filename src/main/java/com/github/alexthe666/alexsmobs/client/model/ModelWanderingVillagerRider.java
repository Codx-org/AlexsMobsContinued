...
public class ModelWanderingVillagerRider extends VillagerModel {
    ...
    //? if >=1.21.2 {
    /*@Override
    public void setupAnim(net.minecraft.client.renderer.entity.state.VillagerRenderState state) {
        super.setupAnim(state);
        // This model is only ever swapped in while the trader is riding an elephant, so the
        // sitting pose is unconditional (the old `riding` model flag no longer exists).
