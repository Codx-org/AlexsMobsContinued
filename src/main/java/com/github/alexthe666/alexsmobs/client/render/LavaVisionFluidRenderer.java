...
public class LavaVisionFluidRenderer extends LiquidBlockRenderer {
    ...
    // 1.21.11 gave LiquidBlockRenderer a MaterialSet constructor parameter (its five fluid sprites
    // are resolved eagerly now instead of through the atlas at draw time). AtlasManager itself
    // implements MaterialSet, so the client's own manager is the exact set vanilla passes — no
    // access transformer on BlockRenderDispatcher#materials needed.
    //? if >=1.21.11 {
    /*public LavaVisionFluidRenderer() {
        super(net.minecraft.client.Minecraft.getInstance().getAtlasManager());
    }
    *///?}

    // 1.20.2 changed TextureAtlasSprite.getU/getV from a double in 0..16 sprite space to a
    // float in normalised 0..1 space. Both call shapes funnel through these two helpers.
    //? if >=1.20.2 {
    /*private static float spriteU(net.minecraft.client.renderer.texture.TextureAtlasSprite sprite, double u16) {
        return sprite.getU((float) (u16 / 16.0D));
    }

    private static float spriteV(net.minecraft.client.renderer.texture.TextureAtlasSprite sprite, double v16) {
        return sprite.getV((float) (v16 / 16.0D));
    }
    *///?}
    //? if <1.20.2 {
    private static float spriteU(net.minecraft.client.renderer.texture.TextureAtlasSprite sprite, double u16) {
        return sprite.getU(u16);
    }

    private static float spriteV(net.minecraft.client.renderer.texture.TextureAtlasSprite sprite, double v16) {
        return sprite.getV(v16);
    }
    //?}
    ...
    private static boolean isFaceOccludedByState(BlockGetter p_239284_0_, Direction p_239284_1_, float p_239284_2_, BlockPos p_239284_3_, BlockState p_239284_4_) {
        if (p_239284_4_.canOcclude()) {
            ...
            // 1.21.2 caches the occlusion shape on the state, so it no longer takes a level or a pos.
            //? if >=1.21.2 {
            /*VoxelShape voxelshape1 = p_239284_4_.getOcclusionShape();
            *///?} else {
    ...
    public void tesselate(BlockAndTintGetter lightReaderIn, BlockPos posIn, VertexConsumer vertexBuilderIn, BlockState blockstateIn, FluidState fluidStateIn) {
        try {
            if (fluidStateIn.is(FluidTags.LAVA)) {
                // Same lookup either way; NeoForge moved it off ClientHooks into its own cache.
                //? if forge {
                ...
                *///?}
                ...
                // 1.21.2 dropped shouldRenderFace's level and pos; the neighbour state it needed
                // them for is already one of the remaining arguments.
                //? if >=1.21.2 {
                /*boolean flag2 = shouldRenderFace(fluidStateIn, blockstateIn, Direction.DOWN, fluidstate) && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, Direction.DOWN, 0.8888889F, blockstate);
                boolean flag3 = shouldRenderFace(fluidStateIn, blockstateIn, Direction.NORTH, fluidstate2);
                boolean flag4 = shouldRenderFace(fluidStateIn, blockstateIn, Direction.SOUTH, fluidstate3);
                boolean flag5 = shouldRenderFace(fluidStateIn, blockstateIn, Direction.WEST, fluidstate4);
                boolean flag6 = shouldRenderFace(fluidStateIn, blockstateIn, Direction.EAST, fluidstate5);
                *///?} else {
                ...
                if (!flag1 && !flag2 && !flag6 && !flag5 && !flag3 && !flag4) {
                    ...
                    if (flag1 && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, Direction.UP, Math.min(Math.min(f8, f10), Math.min(f9, f7)), blockstate1)) {
                        ...
                        if (vec3.x == 0.0D && vec3.z == 0.0D) {
                            ...
                            f18 = spriteU(textureatlassprite1, 0.0D);
                            f22 = spriteV(textureatlassprite1, 0.0D);
                            f19 = f18;
                            f23 = spriteV(textureatlassprite1, 16.0D);
                            f20 = spriteU(textureatlassprite1, 16.0D);
                            f24 = f23;
                            f21 = f20;
                            f25 = f22;
                        } else {
                            ...
                            f18 = spriteU(textureatlassprite, (double)(8.0F + (-f28 - f27) * 16.0F));
                            f22 = spriteV(textureatlassprite, (double)(8.0F + (-f28 + f27) * 16.0F));
                            f19 = spriteU(textureatlassprite, (double)(8.0F + (-f28 + f27) * 16.0F));
                            f23 = spriteV(textureatlassprite, (double)(8.0F + (f28 + f27) * 16.0F));
                            f20 = spriteU(textureatlassprite, (double)(8.0F + (f28 + f27) * 16.0F));
                            f24 = spriteV(textureatlassprite, (double)(8.0F + (f28 - f27) * 16.0F));
                            f21 = spriteU(textureatlassprite, (double)(8.0F + (f28 - f27) * 16.0F));
                            f25 = spriteV(textureatlassprite, (double)(8.0F + (-f28 - f27) * 16.0F));
                        }
                    ...
                    for(Direction direction : Direction.Plane.HORIZONTAL) {
                        ...
                        if (flag8 && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, direction, Math.max(f44, f45), lightReaderIn.getBlockState(posIn.relative(direction)))) {
                            ...
                            float f54 = spriteU(textureatlassprite2, 0.0D);
                            float f55 = spriteU(textureatlassprite2, 8.0D);
                            float f33 = spriteV(textureatlassprite2, (double)((1.0F - f44) * 16.0F * 0.5F));
                            float f34 = spriteV(textureatlassprite2, (double)((1.0F - f45) * 16.0F * 0.5F));
                            float f35 = spriteV(textureatlassprite2, 8.0D);
    ...
    private void vertexVanilla(VertexConsumer vertexBuilderIn, double x, double y, double z, float red, float green, float blue, float alpha, float u, float v, int packedLight) {
        // 1.21's addVertex takes floats; the pre-1.21 vertex(double, double, double) widens them
        // back, so casting here is version-neutral.
        AMVertex.normal(vertexBuilderIn.vertex((float) x, (float) y, (float) z).color(red, green, blue, alpha).uv(u, v).uv2(packedLight), 0.0F, 1.0F, 0.0F).endVertex();
    }
    ...
    private static boolean isFaceOccludedByStateVanilla(BlockGetter p_110979_, Direction p_110980_, float p_110981_, BlockPos p_110982_, BlockState p_110983_) {
        if (p_110983_.canOcclude()) {
            ...
            // 1.21.2 caches the occlusion shape on the state, so it no longer takes a level or a pos.
            //? if >=1.21.2 {
            /*VoxelShape voxelshape1 = p_110983_.getOcclusionShape();
            *///?} else {
