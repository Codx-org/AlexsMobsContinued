...
public abstract class LayerVoidWormGlow<T extends LivingEntity> extends RenderLayer<T, EntityModel<T>>  {
    ...
    public LayerVoidWormGlow(
            ...
            *///?} else {
            ...
            //?}
            ResourceManager resourceManager, EntityModel<T> layerModel) {
        super(renderer);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T worm, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (isGlowing(worm) || special) {
            if(special){
                ...
                AMRenderTypes.renderMerged(bufferIn, AMRenderTypes.VOID_WORM_PORTAL_OVERLAY, RenderType.entityCutoutNoCull(texture),
                        consumer -> com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.renderToBuffer(this.layerModel, matrixStackIn, consumer, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F));
            }else{
                float f = getAlpha(worm);
                com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.renderToBuffer(this.getParentModel(), matrixStackIn, bufferIn.getBuffer(RenderType.eyes(texture)), 240, LivingEntityRenderer.getOverlayCoords(worm, 1.0F), 1.0F, 1.0F, 1.0F, f);
            }
        }
    }
