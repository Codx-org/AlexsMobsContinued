...
// BakedModel is only referenced by the <1.21.4 render path below; the class was removed in 1.21.5.
//? if <1.21.4 {
...
public class RenderCapsid<T extends TileEntityCapsid> implements BlockEntityRenderer<T> {
    ...
    @Override
    //? if >=1.21.5 {
    /*public void render(T entity, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, net.minecraft.world.phys.Vec3 amCamPos) {
    *///?} else {
    ...
    //?}
        ...
        if (!stack.isEmpty()) {
            ...
            //? if >=1.21.4 {
            /*// 1.21.4 removed ItemRenderer#getModel/isGui3d; capsid items render via renderStatic below and
            // are treated as flat (the gui3d fan layout is dropped — cosmetic).
            boolean flag = false;
            *///?} else {
            ...
            for(int k = 0; k < j; ++k) {
                ...
                //? if >=1.21.4 {
                /*com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.renderItemStatic(stack, ItemDisplayContext.GROUND, combinedLightIn, OverlayTexture.NO_OVERLAY, matrixStackIn, bufferIn, entity.getLevel(), 0);
                *///?} else {
