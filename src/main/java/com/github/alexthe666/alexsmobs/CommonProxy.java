...
public class CommonProxy {
    ...
    //? if >=1.21.2 {
    /*// Rebuilt every reload with the current registries — datapack registry contents can change
    // between reloads, and the codec needs them to resolve item/tag ingredients.
    public CapsidRecipeManager getCapsidRecipeManager(net.minecraft.core.HolderLookup.Provider registries){
        capsidRecipeManager = new CapsidRecipeManager(registries);
    ...
    *///?}
    public CapsidRecipeManager getCapsidRecipeManager(){
        if(capsidRecipeManager == null){
            //? if >=1.21.2 {
            /*capsidRecipeManager = new CapsidRecipeManager(net.minecraft.core.RegistryAccess.EMPTY);
            *///?} else {
