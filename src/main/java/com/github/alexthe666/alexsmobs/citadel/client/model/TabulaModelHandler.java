package com.github.alexthe666.alexsmobs.citadel.client.model;

import com.github.alexthe666.alexsmobs.misc.AMCompat;

import com.github.alexthe666.alexsmobs.citadel.client.model.container.TabulaCubeContainer;
import com.github.alexthe666.alexsmobs.citadel.client.model.container.TabulaCubeGroupContainer;
//? if <26 {
import com.github.alexthe666.alexsmobs.citadel.client.model.container.TabulaModelBlock;
//?}
import com.github.alexthe666.alexsmobs.citadel.client.model.container.TabulaModelContainer;
...
public enum TabulaModelHandler implements JsonDeserializationContext {
    ...
    // The two ItemTransform adapters only ever served the vanilla-block-model path (loadModel,
    // long since commented out); `gson` itself is still live for the .tbl containers. MC 26.1
    // moved both types AND deleted the container classes' supertypes, so on >=26 the whole
    // block-model half of this class is excluded from the build — see ModPlatformPlugin.
    //? if >=26 {
  ...
  /*  @Override
    ...
    public IUnbakedModel loadModel(ResourceLocation modelLocation) throws Exception {
        ...
        IResource resource = this.manager.getResource(AMCompat.rl(modelLocation.getPath(), modelPath));
        ...
        ResourceLocation tblLocation = AMCompat.rl(tblLocationStr);
        ...
        while ((texture = TabulaModelBlock.textures.get("layer" + layer++)) != null) {
            builder.add(AMCompat.rl(texture));
        }
        String particle = TabulaModelBlock.textures.get("particle");
        return new VanillaTabulaModel(modelJson, particle != null ? AMCompat.rl(particle) : null, builder.build(), PerspectiveMapWrapper.getTransforms(TabulaModelBlock.getAllTransforms()));
    }
