...
public class TabulaModelBlock {
    ...
    // 1.21.4 removed ItemOverride/ItemOverrides (item model definitions replace them). This whole
    // overrides machinery is dead code — TabulaModelBlock is only reachable through the commented-out
    // geometry loader in TabulaModelHandler — so on >=1.21.4 it is stubbed out with Object placeholders.
    @VisibleForTesting
    // 1.21.5 removed BlockFaceUV (folded into the BlockElementFace$UVs record + Quadrant), so its
    // Gson type-adapter registration is dropped there. All the other adapters still exist.
    //? if >=1.21.5 {
    ...
    *///?} else {
    ...
    //? if >=1.21.4 {
    /*private final List<Object> overrides;
    *///?} else {
    ...
    //? if >=1.21.4 {
    /*public TabulaModelBlock(@Nullable ResourceLocation parentLocationIn, List<BlockElement> elementsIn, Map<String, String> texturesIn, boolean ambientOcclusionIn, boolean gui3dIn, ItemTransforms cameraTransformsIn, List<Object> overridesIn) {
    *///?} else {
    ...
    public Collection<ResourceLocation> getOverrideLocations() {
        ...
        //? if >=1.21.4 {
        /*// overrides removed in 1.21.4 — nothing to collect
        *///?} elif >=1.21.2 {
        /*for (ItemOverride itemoverride : this.overrides) {
            set.add(itemoverride.model());
        }
        *///?} else {
        for (ItemOverride itemoverride : this.overrides) {
    ...
    *///?} else {
    ...
    public ItemTransforms getAllTransforms() {
        ...
        // 1.21.9 added a ninth display context (FIXED_FROM_BOTTOM). Tabula models have no such
        // transform, so it gets the identity — which is what the old eight-arg form implied anyway.
        //? if >=1.21.9 {
    ...
    private ItemTransform getTransform(ItemDisplayContext type) {
        //? if >=1.21.4 {
        /*// 1.21.4 removed ItemTransforms#hasTransform; NO_TRANSFORM sentinel means "no transform present".
        return this.parent != null && this.cameraTransforms.getTransform(type) == ItemTransform.NO_TRANSFORM ? this.parent.getTransform(type) : this.cameraTransforms.getTransform(type);
        *///?} else {
    ...
    public static class Deserializer implements JsonDeserializer<TabulaModelBlock> {
        public TabulaModelBlock deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
            ...
            //? if >=1.21.4 {
            /*List<Object> list1 = this.getItemOverrides(p_deserialize_3_, jsonobject);
            *///?} else {
            List<ItemOverride> list1 = this.getItemOverrides(p_deserialize_3_, jsonobject);
            //?}
            ResourceLocation resourcelocation = s.isEmpty() ? null : AMCompat.rl(s);
            return new TabulaModelBlock(resourcelocation, list, map, flag, true, itemcameratransforms, list1);
        }

        //? if >=1.21.4 {
        /*protected List<Object> getItemOverrides(JsonDeserializationContext deserializationContext, JsonObject object) {
            // ItemOverride removed in 1.21.4; dead path, so return empty.
