...
import com.github.alexthe666.alexsmobs.citadel.server.entity.pathfinding.raycoms.pathjobs.AbstractPathJob;
import com.github.alexthe666.alexsmobs.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobMoveAwayFromLocation;
import com.github.alexthe666.alexsmobs.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobMoveToLocation;
import com.github.alexthe666.alexsmobs.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobRandomPos;
import com.github.alexthe666.alexsmobs.citadel.server.world.WorldChunkUtil;
...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    public void tick() {
        ...
        if (!this.isDone()) {
            ...
            // 1.21.9 DELETED DebugPackets; the F3+Y path overlay is fed by Mob#registerDebugValues,
            // which publishes DebugSubscriptions.ENTITY_PATHS from a DebugPathInfo(path, maxDistanceToWaypoint)
            // for every mob — i.e. exactly this payload, so nothing is lost by dropping the send.
            //? if <1.21.9 {
    ...
    private boolean handleLadders(int oldIndex) {
        ...
        if (!this.isDone()) {
            ...
            if (pEx.isOnLadder() && pExNext != null && (pEx.y != pExNext.y || mob.getY() > pEx.y) && com.github.alexthe666.alexsmobs.misc.AMCompat.isLadder(level.getBlockState(pos), level, pos, ourEntity)) {
    ...
    private boolean handlePathPointOnLadder(final PathPointExtended pEx) {
        ...
        if (vec3.distanceToSqr(ourEntity.getX(), vec3.y, ourEntity.getZ()) < 0.6 && Math.abs(vec3.y - entityPos.getY()) <= 2.0) {
            ...
            if (newSpeed > 0) {
                ...
                if (com.github.alexthe666.alexsmobs.misc.AMCompat.isLadder(level.getBlockState(entityPos.below()), level, entityPos.below(), ourEntity)) {
