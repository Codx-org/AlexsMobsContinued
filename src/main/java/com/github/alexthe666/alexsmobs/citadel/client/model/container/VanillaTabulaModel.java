...
public class VanillaTabulaModel implements UnbakedModel {
    ...
    // 1.21.5 made UnbakedModel's resolve/bake methods defaults (or removed them), so no override is
    // needed on this dead class; earlier eras still require the era-appropriate override.
    //? if >=1.21.5 {
    /*
    *///?} elif >=1.21.2 {
    /*@Override
    public void resolveDependencies(UnbakedModel.Resolver p_119538_) {
    ...
    *///?} else {
    ...
    // 1.21.4 rewrote bake to (TextureSlots, ModelBaker, ModelState, boolean, boolean, ItemTransforms)
    // as part of the item-model-definitions rework. This class is dead code (only referenced from the
    // commented-out geometry loader in TabulaModelHandler), so a null-returning stub keeps it compiling.
    //? if >=1.21.5 {
    /*// bake() (returning the removed BakedModel) is gone from UnbakedModel in 1.21.5; no override.
    *///?} elif >=1.21.4 {
    /*@org.jetbrains.annotations.Nullable
    @Override
    public BakedModel bake(net.minecraft.client.renderer.block.model.TextureSlots slots, ModelBaker baker, ModelState state, boolean b1, boolean b2, net.minecraft.client.renderer.block.model.ItemTransforms transforms) {
        return null;
    }
    *///?} elif >=1.21 {
    /*// 1.21 dropped the trailing ResourceLocation ("which model am I") argument from bake.
    @org.jetbrains.annotations.Nullable
    @Override
...
/*
    ...
    public IBakedModel bake(ModelBakery bakery, Function spriteGetter, ISprite sprite, VertexFormat format) {
        TextureAtlasSprite spriteA = (TextureAtlasSprite) spriteGetter.apply(this.textures.isEmpty() ? AMCompat.rl("missingno") : this.textures.get(0));
