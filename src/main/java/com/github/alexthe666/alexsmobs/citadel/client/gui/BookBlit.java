...
public class BookBlit {
    ...
    private static void blitWithColor(GuiGraphics guiGraphics, ResourceLocation texture, int startX, int endX, int startY, int endY, int zLevel, float u0, float u1, float v0, float v1, int r, int g, int b, int a) {
        //? if >=1.21.5 {
        /*// 1.21.5 removed the immediate-mode RenderSystem.setShaderTexture/BufferUploader path.
        // Funnel through the modern tinted GuiGraphics.blit. This MUST go through the uv-taking
        // helper: the book's own background blits stretch a whole texture across a 390x320 frame,
        // i.e. destination size != source size, which the 1:1 blitTinted form cannot express.
        com.github.alexthe666.alexsmobs.client.render.AMRenderCompat.blitTintedUV(guiGraphics, texture, startX, startY, endX, endY, u0, u1, v0, v1, net.minecraft.util.ARGB.color(a, r, g, b));
        *///?} else {
