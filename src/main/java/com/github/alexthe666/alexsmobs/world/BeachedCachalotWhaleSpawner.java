...
// 1.20.5 replaced the SpawnPlacements.Type enum with the SpawnPlacementType interface, and
// moved the position check off NaturalSpawner onto the placement type itself.
//? if >=1.20.5
...
public class BeachedCachalotWhaleSpawner {
    ...
    private boolean attemptSpawnWhale() {
        ...
        if (playerentity == null) {
            ...
            if (blockpos2 != null && this.func_226559_a_(blockpos2) && blockpos2.distSqr(blockpos) > 225) {
                ...
                EntityCachalotWhale whale = AMCompat.create(AMEntityRegistry.CACHALOT_WHALE.get(), world);
    ...
    private BlockPos func_221244_a(BlockPos p_221244_1_, int p_221244_2_) {
        ...
        for(int i = 0; i < 10; ++i) {
            ...
            if (AMWorldRegistry.testBiome(BiomeConfig.cachalot_whale_beached_spawns, world.getBiome(blockpos1)) && AMCompat.isSpawnPositionOnGround(this.world, blockpos1, EntityType.WANDERING_TRADER)) {
                blockpos = blockpos1;
                break;
            }
        }
