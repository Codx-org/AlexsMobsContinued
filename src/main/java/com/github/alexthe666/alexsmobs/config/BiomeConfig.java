...
import com.github.alexthe666.alexsmobs.citadel.config.biome.SpawnBiomeConfig;
...
public class BiomeConfig {
        ...
        try {
            for (Field f : BiomeConfig.class.getDeclaredFields()) {
               ...
               if(obj instanceof Pair){
				   ...
				   biomeConfigValues.put(id, SpawnBiomeConfig.create(AMCompat.rl(id), data));
               }
            }
        }catch (Exception e){
	...
	public static boolean test(Pair<String, SpawnBiomeData> spawns, Holder<Biome> biome) {
		return test(spawns, biome, biome.unwrapKey().map(ResourceKey::location).orElse(null));
	}
}
