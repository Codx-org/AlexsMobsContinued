...
import com.github.alexthe666.alexsmobs.citadel.config.biome.BiomeEntryType;
...
public class DefaultBiomes {
    ...
    public static final SpawnBiomeData KOMODO_DRAGON = new SpawnBiomeData()
            ...
            // The one narrowed convention tag that cannot be repaired by widening a tag, because it
            // is NEGATED in pool 0: `c:is_dense_vegetation/overworld` ADDED bamboo_jungle (and
            // mangrove swamp) to what `forge:is_dense/overworld` held, which SUBTRACTS bamboo_jungle
            // from that pool and halves the komodo's vanilla range. Restored as its own pool — on
            // the nodes still reading `forge:` that biome already matched pool 0, so this is a no-op
            // there rather than a divergence. (Pool indices must be added in non-decreasing order:
            // addBiomeEntry grows the list by exactly one, so a forward jump would throw.)
            .addBiomeEntry(BiomeEntryType.REGISTRY_NAME, false, "minecraft:bamboo_jungle", 5);
