...
public class TileEntityEndPirateAnchorWinch extends BlockEntity {
    ...
    private int calcChainLength(boolean goBelowAnchor) {
        ...
        while (level != null && down.getY() > AMCompat.minBuildHeight(level) && !isAnchorTop(level, down) && (isEmptyBlock(down) || isAnchorChain(level, down))) {
            down = down.below();
        }
        int i = 0;
    ...
    private int keepMovingBelowAnchor(BlockPos below) {
        while (below.getY() > AMCompat.minBuildHeight(level) && isEmptyBlock(below)) {
            below = below.below();
        }
        return below.getY();
    }
