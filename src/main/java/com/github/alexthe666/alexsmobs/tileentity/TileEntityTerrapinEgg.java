...
public class TileEntityTerrapinEgg extends BlockEntity {
    ...
    *///?} else {
    ...
    //?}
        ...
        //? if <1.20.5
        super.load(compound);
        if(AMCompat.contains(compound, "Parent1Data")){
            this.parent1 = new ParentData(AMCompat.getCompound(compound, "Parent1Data"));
        }
        if(AMCompat.contains(compound, "Parent2Data")){
            this.parent2 = new ParentData(AMCompat.getCompound(compound, "Parent2Data"));
        }
    }
    ...
    *///?} else {
    ...
    //?}
        ...
        if(this.parent1 != null){
            ...
            AMCompat.put(compound, "Parent1Data", tag);
        }
        if(this.parent2 != null){
            CompoundTag tag = new CompoundTag();
            parent2.writeToNBT(tag);
            AMCompat.put(compound, "Parent2Data", tag);
        }
    }
    ...
    public static class ParentData {
        ...
        public ParentData(CompoundTag tag){
            this(TerrapinTypes.values()[Mth.clamp(AMCompat.getInt(tag, "TerrapinType"), 0, TerrapinTypes.values().length - 1)],
                    AMCompat.getInt(tag, "ShellType"),
                    AMCompat.getInt(tag, "SkinType"),
                    AMCompat.getInt(tag, "TurtleColor"),
                    AMCompat.getInt(tag, "ShellColor"),
                    AMCompat.getInt(tag, "SkinColor")
                    );
        }
