...
class Context(
	...
	// Fabric-only manifest fields. Optional so a tree with no Fabric node needs no such keys;
	// a Fabric node with no entrypoint would load and do nothing, which is why generateManifest
	// treats a blank main entrypoint as a hard error rather than emitting an empty list.
	val fabricMainEntrypoint: String by lazy { optional("mod.fabric.entrypoint_main") }
	val fabricClientEntrypoint: String by lazy { optional("mod.fabric.entrypoint_client") }
	val fabricAccessWidener: String by lazy { optional("mod.fabric.access_widener") }

	// Fabric's convention is assets/<modid>/icon.png, but a tree that started life as a Forge
	// mod keeps its logo wherever the Forge manifest's logoFile pointed. Overridable so the
	// same PNG serves both manifests instead of being duplicated into every jar.
	val fabricIcon: String by lazy { optional("mod.fabric.icon", "assets/$modId/icon.png") }
